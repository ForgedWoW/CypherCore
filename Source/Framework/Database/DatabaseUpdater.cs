...
public class DatabaseUpdater<T>
{
    ...
    public bool Populate()
    {
        ...
        {
            Log.Logger.Error($"File \"{path + fileName}\" is missing, download it from \"https://github.com/TrinityCore/TrinityCore/releases\"" +
        ...
        foreach (var availableQuery in availableFiles)
        {
            ...
            if (applied == null)
            {
                ...
                if (hashIter != null)
                {
                    ...
                    if (renameFile != null)
                    {
                        Log.Logger.Warning($"Seems like update \"{availableQuery.GetFileName()}\" \'{hash.Substring(0, 7)}\' was renamed, but the old file is still there! " +
