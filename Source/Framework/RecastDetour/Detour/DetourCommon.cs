...
public static partial class Detour
{
    ...
    {
        var pqx = q.GetAt(0) - p.GetAt(0);
        var pqz = q.GetAt(2) - p.GetAt(2);
        var dx = pt[ptStart + 0] - p.GetAt(0);
        var dz = pt[ptStart + 2] - p.GetAt(2);
        var d = pqx * pqx + pqz * pqz;
        t = pqx * dx + pqz * dz;
        if (d > 0) t /= d;
    ...
    {
        var result = float.IsFinite(v[0]) && float.IsFinite(v[2]);
