...
public class SharedConst
{
    ...
    public const uint CalendarDefaultResponseTime = 946684800;
    ...
    ///     Object Const
    /// </summary>
    public const float DefaultPlayerBoundingRadius = 0.388999998569489f;

    // player size, also currently used (correctly?) for any non Unit world objects
    public const float DefaultPlayerCombatReach = 1.5f;

    public const float DefaultVisibilityBGAreans = 533.0f;

    public const float DefaultVisibilityDistance = VisibilityDistanceNormal;

    // default visible distance, 100 yards on continents
    public const float DefaultVisibilityInstance = 170.0f;

    // default visible distance in instances, 170 yards
    // default visible distance in BG/Arenas, roughly 533 yards
    public const int DefaultVisibilityNotifyPeriod = 1000;

    public const uint DisplayIdHiddenMount = 73200;

    public const float ExtraCellSearchRadius = 40.0f;
    ...
    public const int MaxClientMailItems = 12;
    ...
    // client supported items unknown, but provided number must be enough
    ...
    // max number of items a player is allowed to attach
    ...
    public const float MaxVisibilityDistance = MapConst.SizeofGrids;
    ...
    public const float terminal_fallTime = (float)(terminalVelocity / gravity);
    ...
    public const float terminal_safeFall_fallTime = (float)(terminalSafefallVelocity / gravity);
