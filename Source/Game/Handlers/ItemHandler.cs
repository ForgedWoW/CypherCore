...
namespace Game
{
    public partial class WorldSession
    {
                ...
                if (pProto != null)
                {
                    if (pProto.GetSellPrice() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            pl.ItemRemovedQuestCheck(pItem.Entry, pItem.GetCount());
        ...
        void HandleBuybackItem(BuyBackItem packet)
        {
            ...
            if (pItem != null)
            {
                ...
                if (msg == InventoryResult.Ok)
                {
                    ...
                    _player.ItemAddedQuestCheck(pItem.Entry, pItem.GetCount());
        ...
        void HandleBuyItem(BuyItem packet)
        {
            ...
            switch (packet.ItemType)
            {
                case ItemVendorType.Item:
                    Item bagItem = Player.GetItemByGuid(packet.ContainerGUID);
        ...
        void HandleWrapItem(WrapItem packet)
        {
            ...
            Item gift = Player.GetItemByPos(giftContainerSlot, giftSlot);
            if (!gift)
            {
            ...
            Item item = Player.GetItemByPos(itemContainerSlot, itemSlot);
            if (!item)
            {
        ...
        void HandleSocketGems(SocketGems socketGems)
        {
            ...
            Item itemTarget = Player.GetItemByGuid(socketGems.ItemGuid);
            ...
            for (int i = 0; i < ItemConst.MaxGemSockets; ++i)
            {
                ...
                if (iGemProto.HasFlag(ItemFlags.UniqueEquippable))
                {
                    for (int j = 0; j < ItemConst.MaxGemSockets; ++j)
                    {
                        ...
                        if (gems[j])
                        {
                            if (iGemProto.GetId() == gems[j].Entry)
                            {
                ...
                {
                    InventoryResult res = Player.CanEquipUniqueItem(gems[i], slot, (uint)Math.Max(limit_newcount, 0));
                    if (res != 0)
                    {
            ...
            for (ushort i = 0; i < ItemConst.MaxGemSockets; ++i)
            {
                if (gems[i])
                {
                    uint gemScalingLevel = _player.Level;
            ...
            if (SocketBonusActivated ^ SocketBonusToBeActivated)     //if there was a change...
            {
                ...
                itemTarget.SetEnchantment(EnchantmentSlot.Bonus, SocketBonusToBeActivated ? itemTarget.GetTemplate().GetSocketBonus() : 0, 0, 0, Player.GUID);
        ...
        void HandleCancelTempEnchantment(CancelTempEnchantment packet)
        {
            ...
            Item item = Player.GetItemByPos(InventorySlots.Bag0, (byte)packet.Slot);
            if (!item)
                return;
        ...
        bool CanUseBank(ObjectGuid bankerGUID = default)
        {
            ...
            bool isUsingBankCommand = bankerGUID == Player.GUID && bankerGUID == _player.PlayerTalkClass.GetInteractionData().SourceGuid;

            if (!isUsingBankCommand)
            {
                Creature creature = Player.GetNPCIfCanInteractWith(bankerGUID, NPCFlags.Banker, NPCFlags2.None);
                if (!creature)
                    return false;
            }
