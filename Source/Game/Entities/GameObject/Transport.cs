...
class Transport : GameObjectTypeBase, ITransport
{
	...
	public override void Update(uint diff)
	{
		...
		else
		{
			...
			{
				int timeToStop = (int)(Owner.GameObjectData.Level - _stateChangeTime);
		...
		{
			Matrix4x4 pathRotation = new Quaternion(Owner.GameObjectData.ParentRotation.GetValue().X, Owner.GameObjectData.ParentRotation.GetValue().Y,
			                                        Owner.GameObjectData.ParentRotation.GetValue().Z, Owner.GameObjectData.ParentRotation.GetValue().W).ToMatrix();
	...
	public ObjectGuid GetTransportGUID() { return Owner.GetGUID(); }

	public float GetTransportOrientation() { return Owner.Location.Orientation; }
