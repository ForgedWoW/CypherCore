...
public partial class Player
{
	...
	{
		var stmt = CharacterDatabase.GetPreparedStatement(CharStatements.DEL_CHARACTER_CUSTOMIZATIONS);
	...
	public void SaveToDB(SQLTransaction loginTransaction, SQLTransaction characterTransaction, bool create = false)
	{
		...
		else
		{
			...
			{
				stmt.AddValue(index++, (ushort)GetTeleportDest().MapId);
				stmt.AddValue(index++, 0);
				stmt.AddValue(index++, (byte)GetDungeonDifficultyId());
				stmt.AddValue(index++, (byte)GetRaidDifficultyId());
				stmt.AddValue(index++, (byte)GetLegacyRaidDifficultyId());
	...
	public static uint GetZoneIdFromDB(ObjectGuid guid)
	{
		...
		var stmt = CharacterDatabase.GetPreparedStatement(CharStatements.SEL_CHAR_ZONE);
	...
	public static void DeleteOldCharacters(int keepDays)
	{
		...
		var stmt = CharacterDatabase.GetPreparedStatement(CharStatements.SEL_CHAR_OLD_CHARS);
	...
	{
		var stmt = CharacterDatabase.GetPreparedStatement(CharStatements.UPD_CHARACTER_POSITION);
	...
	{
		var stmt = CharacterDatabase.GetPreparedStatement(CharStatements.SEL_CHAR_POSITION);
	...
	void _LoadSkills(SQLResult result)
	{
		...
		if (!result.IsEmpty())
			do
			{
				...
				var rcEntry = Global.DB2Mgr.GetSkillRaceClassInfo(skill, race, GetClass());
	...
	bool _LoadHomeBind(SQLResult result)
	{
		...
		if (!ok)
		{
			var loc = Global.ObjectMgr.GetDefaultGraveYard(GetTeam());
	...
	static Item _LoadMailedItem(ObjectGuid playerGuid, Player player, ulong mailId, Mail mail, SQLFields fields, ItemAdditionalLoadInfo addionalData)
	{
		...
		if (proto == null)
		{
			...
			var stmt = CharacterDatabase.GetPreparedStatement(CharStatements.DEL_INVALID_MAIL_ITEM);
		...
		if (!item.LoadFromDB(itemGuid, ownerGuid, fields, itemEntry))
		{
			...
			var stmt = CharacterDatabase.GetPreparedStatement(CharStatements.DEL_MAIL_ITEM);
			stmt.AddValue(0, itemGuid);
			DB.Characters.Execute(stmt);
	...
	void _LoadStoredAuraTeleportLocations(SQLResult result)
	{
		...
		if (!result.IsEmpty())
			do
			{
				...
				{
					Log.outError(LogFilter.Spells, $"Player._LoadStoredAuraTeleportLocations: Player {GetName()} ({GetGUID()}) spell (ID: {spellId}) has invalid position on map {location.MapId}, {location}.");
