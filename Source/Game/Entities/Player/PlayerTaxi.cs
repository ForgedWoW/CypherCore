...
public class PlayerTaxi
{
	...
	public bool LoadTaxiDestinationsFromString(string values, Team team)
	{
		...
		{
			Global.ObjectMgr.GetTaxiPath(m_TaxiDestinations[i - 1], m_TaxiDestinations[i], out var path, out _);
	...
	public uint GetCurrentTaxiPath()
	{
		...
		Global.ObjectMgr.GetTaxiPath(m_TaxiDestinations[0], m_TaxiDestinations[1], out var path, out _);
	...
	{
		m_TaxiDestinations.Add(dest);
	}

	public uint GetTaxiSource()
	{
		return m_TaxiDestinations.Empty() ? 0 : m_TaxiDestinations[0];
	}

	public uint GetTaxiDestination()
	{
		return m_TaxiDestinations.Count < 2 ? 0 : m_TaxiDestinations[1];
	}
