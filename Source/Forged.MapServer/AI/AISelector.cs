...
public class AISelector
{
    public static CreatureAI SelectAI(Creature creature)
    {
        ...
        //scriptname in db
        var scriptedAI = creature.ScriptManager.RunScriptRet<ICreatureGetAI, CreatureAI>(p => p.GetAI(creature), creature.GetScriptId());
    ...
    {
        // scriptname in db
        var scriptedAI = go.ScriptManager.RunScriptRet<IGameObjectGetAI, GameObjectAI>(p => p.GetAI(go), go.ScriptId);
