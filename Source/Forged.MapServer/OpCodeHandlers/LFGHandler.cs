...
public class LFGHandler : IWorldSessionHandler
{
    ...
    public void SendLfgBootProposalUpdate(LfgPlayerBoot boot)
    {
        ...
        LfgBootPlayer lfgBootPlayer = new()
        {
            Info =
            {
                VoteInProgress = boot.InProgress,                        // Vote in progress
                VotePassed = agreeNum >= SharedConst.LFGKickVotesNeeded, // Did succeed
                MyVoteCompleted = playerVote != LfgAnswer.Pending,       // Did Vote
                MyVote = playerVote == LfgAnswer.Agree,                  // Agree
                Target = boot.Victim,                                    // Victim GUID
                TotalVotes = votesNum,                                   // Total Votes
                BootVotes = agreeNum,                                    // Agree Count
                TimeLeft = secsleft,                                     // Time Left
                VotesNeeded = SharedConst.LFGKickVotesNeeded,            // Needed Votes
                Reason = boot.Reason                                     // Kick reason
            }
        };
    ...
    public void SendLfgPlayerReward(LfgPlayerRewardData rewardData)
    {
        ...
        {
            QueuedSlot = rewardData.RdungeonEntry,
            ActualSlot = rewardData.SdungeonEntry,
            RewardMoney = _session.Player.GetQuestMoneyReward(rewardData.Quest),
            AddedXP = _session.Player.GetQuestXPReward(rewardData.Quest)
        };
    ...
    public void SendLfgProposalUpdate(LfgProposal proposal)
    {
        ...
        foreach (var pair in proposal.Players)
        {
            ...
            {
                Roles = (uint)pair.Value.Role,
                Me = (pair.Key == playerGuid),
                MyParty = !pair.Value.Group.IsEmpty && pair.Value.Group == proposal.Group,
                SameParty = !pair.Value.Group.IsEmpty && pair.Value.Group == guildGuid,
                Responded = (pair.Value.Accept != LfgAnswer.Pending),
                Accepted = (pair.Value.Accept == LfgAnswer.Agree)
            };
    ...
    public void SendLfgRoleCheckUpdate(LfgRoleCheck roleCheck)
    {
        ...
        {
            PartyIndex = 127,
            RoleCheckStatus = (byte)roleCheck.State,
            IsBeginning = roleCheck.State == LfgRoleCheckState.Initialiting
        };
    ...
    {
        if (packet == null || !_session.Player.IsUsingLfg)
            return;
