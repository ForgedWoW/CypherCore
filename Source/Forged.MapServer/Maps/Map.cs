...
public class Map : IDisposable
{
    ...
    public string MapName => Entry.MapName[Global.WorldMgr.DefaultDbcLocale];
    ...
    public Map(uint id, long expiry, uint instanceId, Difficulty spawnmode)
    {
        try
        {
            ...
            _gridExpiry = expiry;
            Terrain = Global.TerrainMgr.LoadTerrain(id);
            ...
            PoolData = Global.PoolMgr.InitPoolsForMap(this);
    ...
    {
        if (checks.HasAnyFlag(LineOfSightChecks.Vmap) && !Global.VMapMgr.IsInLineOfSight(PhasingHandler.GetTerrainMapId(phaseShift, Id, Terrain, x1, y1), x1, y1, z1, x2, y2, z2, ignoreFlags))
            return false;
    ...
    {
        return Terrain.GetAreaInfo(phaseShift, Id, x, y, z, out mogpflags, out adtId, out rootId, out groupId, _dynamicTree);
    }
    ...
    private bool GetEntrancePos(out uint mapid, out float x, out float y)
    {
        ...
        return Entry.GetEntrancePos(out mapid, out x, out y);
    }
    ...
    private Unit GetScriptUnit(WorldObject obj, bool isSource, ScriptInfo scriptInfo)
    {
        ...
        if (obj == null)
        {
            Log.Logger.Error("{0} {1} object is NULL.",
                             scriptInfo.GetDebugInfo(),
                             isSource ? "source" : "target");
        }
        else if (!obj.IsTypeMask(TypeMask.Unit))
        {
            Log.Logger.Error("{0} {1} object is not unit (TypeId: {2}, Entry: {3}, GUID: {4}), skipping.",
                             scriptInfo.GetDebugInfo(),
                             isSource ? "source" : "target",
                             obj.TypeId,
                             obj.Entry,
    ...
    private Player GetScriptPlayer(WorldObject obj, bool isSource, ScriptInfo scriptInfo)
    {
        ...
        else
        {
            ...
            if (player == null)
                Log.Logger.Error("{0} {1} object is not a player (TypeId: {2}, Entry: {3}, GUID: {4}).",
                                 scriptInfo.GetDebugInfo(),
                                 isSource ? "source" : "target",
                                 obj.TypeId,
                                 obj.Entry,
    ...
    private Creature GetScriptCreature(WorldObject obj, bool isSource, ScriptInfo scriptInfo)
    {
        ...
        else
        {
            ...
            if (creature == null)
                Log.Logger.Error("{0} {1} object is not a creature (TypeId: {2}, Entry: {3}, GUID: {4}).",
                                 scriptInfo.GetDebugInfo(),
                                 isSource ? "source" : "target",
                                 obj.TypeId,
                                 obj.Entry,
    ...
    private WorldObject GetScriptWorldObject(WorldObject obj, bool isSource, ScriptInfo scriptInfo)
    {
        ...
        else
        {
            ...
            if (pWorldObject == null)
                Log.Logger.Error("{0} {1} object is not a world object (TypeId: {2}, Entry: {3}, GUID: {4}).",
                                 scriptInfo.GetDebugInfo(),
                                 isSource ? "source" : "target",
                                 obj.TypeId,
                                 obj.Entry,
    ...
    private void ScriptProcessDoor(WorldObject source, WorldObject target, ScriptInfo scriptInfo)
    {
        ...
        else
        {
            if (source == null)
            {
                Log.Logger.Error("{0} source object could not be casted to world object (TypeId: {1}, Entry: {2}, GUID: {3}), skipping.",
                                 scriptInfo.GetDebugInfo(),
                                 source.TypeId,
                                 source.Entry,
                                 source.GUID.ToString());
                ...
                switch (step.Script.command)
                {
                    ...
                    case ScriptCommands.QuestExplored:
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Log.Logger.Error("{0} neither source nor target is player (Entry: {0}, GUID: {1}; target: Entry: {2}, GUID: {3}), skipping.",
                                                 step.Script.GetDebugInfo(),
                                                 source.Entry,
                                                 source.GUID.ToString(),
                                                 target.Entry,
