	...
	public class LootGroup // A set of loot definitions for items (refs are not allowed)
	{
		...
		public void Verify(LootStore lootstore, uint id, byte group_id = 0)
		{
			...
			if (chance > 101.0f) // @todo replace with 100% when DBs will be ready
				Log.Logger.Error("Table '{0}' entry {1} group {2} has total chance > 100% ({3})", lootstore.GetName(), id, group_id, chance);

			if (chance >= 100.0f && !EqualChanced.Empty())
				Log.Logger.Error("Table '{0}' entry {1} group {2} has items with chance=0% but group total chance >= 100% ({3})", lootstore.GetName(), id, group_id, chance);
		}
