...
public class ItemAdditionalLoadInfo
{
    ...
    public static void Init(Dictionary<ulong, ItemAdditionalLoadInfo> loadInfo, SQLResult artifactResult, SQLResult azeriteItemResult, SQLResult azeriteItemMilestonePowersResult,
                            SQLResult azeriteItemUnlockedEssencesResult, SQLResult azeriteEmpoweredItemResult, CliDB cliDB, DB2Manager db2Manager)
    {
            ...
            do
            {
                ...
                for (var i = 0; i < info.AzeriteItem.SelectedAzeriteEssences.Length; ++i)
                {
                    ...
                    for (var j = 0; j < SharedConst.MaxAzeriteEssenceSlot; ++j)
                    {
                        ...
                        if (azeriteEssence == null || !db2Manager.IsSpecSetMember(azeriteEssence.SpecSetID, specializationId))
                            continue;
        ...
        if (!azeriteItemUnlockedEssencesResult.IsEmpty())
            do
            {
                var azeriteEssencePower = db2Manager.GetAzeriteEssencePower(azeriteItemUnlockedEssencesResult.Read<uint>(1), azeriteItemUnlockedEssencesResult.Read<uint>(2));
