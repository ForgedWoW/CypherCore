...
public partial class Player
{
    ...
    public void AddItemToBuyBackSlot(Item pItem)
    {
        if (pItem != null)
        {
            ...
            if (_items[slot] != null)
            {
                var oldestTime = ActivePlayerData.BuybackTimestamp[0];
                uint oldestSlot = InventorySlots.BuyBackStart;
                ...
                for (byte i = InventorySlots.BuyBackStart + 1; i < InventorySlots.BuyBackEnd; ++i)
                {
                    ...
                    var iTime = ActivePlayerData.BuybackTimestamp[i - InventorySlots.BuyBackStart];
    ...
    {
        var spellInfo = SpellManager.GetSpellInfo(artifactPowerRank.SpellID);
        ...
        if (spellInfo.IsPassive)
        {
            ...
            else if (apply)
            {
                ...
                SpellFactory.CastSpell(this, artifactPowerRank.SpellID, args);
            }
        }
        else
        {
    ...
    public void ApplyAzeriteItemMilestonePower(AzeriteItem item, AzeriteItemMilestonePowerRecord azeriteItemMilestonePower, bool apply)
    {
        ...
        if (!CliDB.AzeritePowerStorage.TryGetValue((uint)azeriteItemMilestonePower.AzeritePowerID, out var azeritePower))
            return;
    ...
    public void ApplyEquipSpell(SpellInfo spellInfo, Item item, bool apply, bool formChange = false)
    {
        if (apply)
        {
            ...
            SpellFactory.CastSpell(this, spellInfo.Id, new CastSpellExtraArgs(item));
        }
        else
        {
    ...
    public void ApplyItemLootedSpell(Item item, bool apply)
    {
        ...
        if (apply)
            SpellFactory.CastSpell(item, (uint)lootedEffect.SpellID);
        else
            RemoveAurasDueToItemSpell((uint)lootedEffect.SpellID, item.GUID);
    }

    public void AutoStoreLoot(uint lootID, LootStore store, ItemContext context = 0, bool broadcast = false, bool createdByPlayer = false)
    {
        AutoStoreLoot(ItemConst.NullBag, ItemConst.NullSlot, lootID, store, context, broadcast);
    }
    ...
    public bool BuyItemFromVendorSlot(ObjectGuid vendorguid, uint vendorslot, uint item, byte count, byte bag, byte slot)
    {
        ...
        var pProto = ItemTemplateCache.GetItemTemplate(item);
    ...
    public InventoryResult CanBankItem(byte bag, byte slot, List<ItemPosCount> dest, Item pItem, bool swap, bool notLoading = true, bool reagentBankOnly = false)
    {
        if (pItem == null)
        ...
        {
            if (slot is >= InventorySlots.BagStart and < InventorySlots.BagEnd)
            {
                if (!pItem.IsBag)
                ...
                res = CanUseItem(pItem, notLoading);
    ...
    public InventoryResult CanEquipItem(byte slot, out ushort dest, Item pItem, bool swap, bool notLoading = true)
    {
        dest = 0;
        ...
        if (pItem != null)
        {
            ...
            if (pProto != null)
            {
                ...
                if (requiredLevels is { MaxLevel: < SharedConst.DefaultMaxLevel } && requiredLevels.Value.MaxLevel < Level && DB2Manager.GetHeirloomByItemId(pProto.Id) == null)
    ...
    public InventoryResult CanEquipUniqueItem(Item pItem, byte eslot = ItemConst.NullSlot, uint limitCount = 1)
    {
        var pProto = pItem.Template;

        // proto based limitations
        var res = CanEquipUniqueItem(pProto, eslot, limitCount);
        ...
        {
            var pGem = ItemTemplateCache.GetItemTemplate(gemData.ItemId);

            if (pGem == null)
                continue;

            // include for check equip another gems with same limit category for not equipped item (and then not counted)
            var gemLimitCount = (uint)(!pItem.IsEquipped && pGem.ItemLimitCategory != 0 ? pItem.GetGemCountWithLimitCategory(pGem.ItemLimitCategory) : 1);

            var ress = CanEquipUniqueItem(pGem, eslot, gemLimitCount);
    ...
    public InventoryResult CanEquipUniqueItem(ItemTemplate itemProto, byte exceptSlot = ItemConst.NullSlot, uint limitCount = 1)
    {
        // check unique-equipped on item
        if (itemProto.HasFlag(ItemFlags.UniqueEquippable))
            // there is an equip limit on this item
            if (HasItemOrGemWithIdEquipped(itemProto.Id, 1, exceptSlot))
    ...
    public InventoryResult CanRollNeedForItem(ItemTemplate proto, Map map, bool restrictOnlyLfg)
    {
        if (restrictOnlyLfg)
        {
            if (Group is not { IsLFGGroup: true })
                return InventoryResult.Ok; // not in LFG group

            // check if looted object is inside the lfg dungeon
            if (!LFGManager.InLfgDungeonMap(Group.GUID, map.Id, map.DifficultyID))
                return InventoryResult.Ok;
        }
    ...
    {
        return pItem == null ? InventoryResult.ItemNotFound : CanStoreItem(bag, slot, dest, pItem.Entry, pItem.Count, pItem, swap);
    }
    ...
    public InventoryResult CanStoreItems(Item[] items, int count, ref uint offendingItemId)
    {
        ...
        for (var i = InventorySlots.ItemStart; i < inventoryEnd; i++)
        {
            ...
            if (item2 is not { IsInTrade: false })
                continue;
        ...
        for (var k = 0; k < count; ++k)
        {
            ...
            var remainingCount = item.Count;

            Log.Logger.Debug($"STORAGE: CanStoreItems {k + 1}. item = {item.Entry}, count = {remainingCount}");
            ...
            if (pProto.MaxStackSize != 1)
            {
                for (var t = InventorySlots.ItemStart; t < inventoryEnd; ++t)
                {
                    ...
                    if (item2 == null || item2.CanBeMergedPartlyWith(pProto) != InventoryResult.Ok || inventoryCounts[t - InventorySlots.ItemStart] >= pProto.MaxStackSize)
                        continue;

                    inventoryCounts[t - InventorySlots.ItemStart] += remainingCount;
                    remainingCount = inventoryCounts[t - InventorySlots.ItemStart] < pProto.MaxStackSize ? 0 : inventoryCounts[t - InventorySlots.ItemStart] - pProto.MaxStackSize;

                    bFound = remainingCount == 0;
                ...
                for (var t = InventorySlots.BagStart; !bFound && t < InventorySlots.BagEnd; ++t)
                {
                    ...
                    for (byte j = 0; j < bag.GetBagSize(); j++)
                    {
                        ...
                        if (item2 == null || item2.CanBeMergedPartlyWith(pProto) != InventoryResult.Ok || bagCounts[t - InventorySlots.BagStart][j] >= pProto.MaxStackSize)
                            continue;

                        // add count to stack so that later items in the list do not double-book
                        bagCounts[t - InventorySlots.BagStart][j] += remainingCount;
                        remainingCount = bagCounts[t - InventorySlots.BagStart][j] < pProto.MaxStackSize ? 0 : bagCounts[t - InventorySlots.BagStart][j] - pProto.MaxStackSize;
    ...
    public InventoryResult CanStoreNewItem(byte bag, byte slot, List<ItemPosCount> dest, uint item, uint count, out uint noSpaceCount)
    {
        return CanStoreItem(bag, slot, dest, item, count, null, false, out noSpaceCount);
    }
    ...
    //UseItem
    public InventoryResult CanUseItem(Item pItem, bool notLoading = true)
    {
        if (pItem == null)
            return InventoryResult.ItemNotFound;
    ...
    public InventoryResult CanUseItem(ItemTemplate proto, bool skipRequiredLevelCheck = false)
    {
        ...
        if (proto.HolidayID != 0 && !GameEventManager.IsHolidayActive(proto.HolidayID))
            return InventoryResult.ClientLockedOut;
    ...
    public void DestroyItem(byte bag, byte slot, bool update)
    {
        ...
        if (pProto != null && pProto.HasFlag(ItemFlags.HasLoot))
            LootItemStorage.RemoveStoredLootForContainer(pItem.GUID.Counter);
        ...
        if (pProto != null && pProto.InventoryType != InventoryType.NonEquip)
    ...
    public uint DestroyItemCount(uint itemEntry, uint count, bool update, bool unequipCheck = true)
    {
        ...
        for (var i = EquipmentSlot.Start; i < InventorySlots.BagEnd; i++)
        {
            ...
            if (item.Count + remcount <= count)
            {
                if (unequipCheck && CanUnequipItem((ushort)(InventorySlots.Bag0 << 8 | i), false) != InventoryResult.Ok)
                    continue;
    ...
    public void DurabilityRepairAll(bool takeCost, float discountMod, bool guildBank)
    {
        ...
        if (guildBank)
        {
            ...
            foreach (var (item, cost) in itemRepairCostStore.OrderByDescending(a => a.cost))
            {
    ...
    public void GearUpByLoadout(uint loadoutPurpose, in List<uint> bonusListIDs)
    {
        ...
        for (var es = EquipmentSlot.Start; es < EquipmentSlot.End; es++)
        {
            ...
            if (GetItemByPos(InventorySlots.Bag0, es) == null)
                continue;

            var offDest = new List<ItemPosCount>();

            if (CanStoreItem(ItemConst.NullBag, ItemConst.NullSlot, offDest, currentEquiped) == InventoryResult.Ok)
            {
    ...
    {
        if (bag == InventorySlots.Bag0 && slot is < (int)PlayerSlots.End and (< InventorySlots.BuyBackStart or >= InventorySlots.BuyBackEnd))
            return _items[slot];
    ...
    public Item GetItemFromBuyBackSlot(uint slot)
    {
        ...
        if (slot is >= InventorySlots.BuyBackStart and < InventorySlots.BuyBackEnd)
            return _items[slot];
    ...
    public uint GetMaxPersonalArenaRatingRequirement(uint minarenaslot)
    {
        ...
        uint maxPersonalRating = 0;

        for (var i = (byte)minarenaslot; i < SharedConst.MaxArenaSlot; ++i)
        {
            var pRating = GetArenaPersonalRating(i);

            if (maxPersonalRating < pRating)
                maxPersonalRating = pRating;
        }

        return maxPersonalRating;
    }
    ...
    public Item GetUseableItemByPos(byte bag, byte slot)
    {
        ...
        return !CanUseAttackType(PlayerComputators.GetAttackBySlot(slot, item.Template.InventoryType)) ? null : item;
    }
    ...
    public bool HasItemCount(uint item, uint count = 1, bool inBankAlso = false)
    {
        ...
        return !ForEachItem(location,
                            pItem =>
                            {
                                if (pItem == null || pItem.Entry != item || pItem.IsInTrade)
                                    return true;
    ...
    public bool HasItemOrGemWithIdEquipped(uint item, uint count, byte exceptSlot = ItemConst.NullSlot)
    {
        uint tempcount = 0;
    ...
    public bool HasItemTotemCategory(uint totemCategory)
    {
        ...
        for (var i = EquipmentSlot.Start; i < inventoryEnd; ++i)
        {
            ...
            if (item != null && DB2Manager.IsTotemCategoryCompatibleWith(item.Template.TotemCategory, totemCategory))
                return true;
        }
    ...
    public bool IsValidPos(byte bag, byte slot, bool explicitPos)
    {
        ...
        {
            // post selected
            case ItemConst.NullBag when !explicitPos:
                return true;
            case InventorySlots.Bag0:
            {
                switch (slot)
                {
                    // any post selected
                    case ItemConst.NullSlot when !explicitPos:
                    // equipment
                    case < EquipmentSlot.End:
                    // profession equipment
                    case >= ProfessionSlots.Start and < ProfessionSlots.End:
                    // bag equip slots
                    case >= InventorySlots.BagStart and < InventorySlots.BagEnd:
                    // reagent bag equip slots
                    case >= InventorySlots.ReagentBagStart and < InventorySlots.ReagentBagEnd:
                        return true;
                }
        ...
        // any post selected
        if (slot == ItemConst.NullSlot && !explicitPos)
            return true;
    ...
    public void MoveItemToInventory(List<ItemPosCount> dest, Item pItem, bool update, bool inCharacterInventoryDB = false)
    {
    ...
    public void RemovedInsignia(Player looterPlr)
    {
        ...
        bones.Loot = LootFactory.GenerateLoot(Location.Map, bones.GUID, LootType.Insignia, looterPlr.Group);
    ...
    public void RemoveItemFromBuyBackSlot(uint slot, bool del)
    {
        ...
        if (slot is >= InventorySlots.BuyBackStart and < InventorySlots.BuyBackEnd)
        {
            ...
            if (_items[_currentBuybackSlot] != null)
    ...
    public void SendEquipError(InventoryResult msg, Item item1 = null, Item item2 = null, uint itemId = 0)
    {
        ...
        if (msg != InventoryResult.Ok)
        {
            ...
            switch (msg)
            {
                ...
                {
                    failure.Level = item1?.GetRequiredLevel() ?? 0;
                ...
                {
                    var proto = item1 != null ? item1.Template : ItemTemplateCache.GetItemTemplate(itemId);
    ...
    public void SendNewItem(Item item, uint quantity, bool pushed, bool created, bool broadcast = false, uint dungeonEncounterId = 0)
    {
        ...
        if (broadcast && Group != null && !item.Template.HasFlag(ItemFlags3.DontReportLootLogToParty))
    ...
    public void SplitItem(ushort src, ushort dst, uint count)
    {
        ...
        if (IsInventoryPos(dst))
        {
            ...
            List<ItemPosCount> dest = new();
            var msg = CanStoreItem(dstbag, dstslot, dest, pNewItem);
    ...
    public void StoreLootItem(ObjectGuid lootWorldObjectGuid, byte lootSlot, Loot loot, AELootResult aeResult = null)
    {
        ...
        if (msg == InventoryResult.Ok)
        {
            ...
            if (ItemTemplateCache.GetItemTemplate(item.Itemid) != null)
            ...
            if (loot.LootType == LootType.Item)
                LootItemStorage.RemoveStoredLootItemForContainer(lootWorldObjectGuid.Counter, item.Itemid, item.Count, item.LootListId);
    ...
    public Item StoreNewItem(List<ItemPosCount> pos, uint itemId, bool update, uint randomBonusListId = 0, List<ObjectGuid> allowedLooters = null, ItemContext context = 0, List<uint> bonusListIDs = null, bool addToCollection = true)
    {
        ...
        if (item != null)
        {
            ...
            {
                var childTemplate = ItemTemplateCache.GetItemTemplate(childItemEntry.ChildItemID);
    ...
    public void SwapItem(ushort src, ushort dst)
    {
        ...
        if (PlayerComputators.IsEquipmentPos(src) || PlayerComputators.IsBagPos(src))
        {
            ...
            var inventoryResult = CanUnequipItem(src, !PlayerComputators.IsBagPos(src) || PlayerComputators.IsBagPos(dst) || (pDstItem is { AsBag: { } } && pDstItem.AsBag.IsEmpty()));

            if (inventoryResult != InventoryResult.Ok)
            {
                SendEquipError(inventoryResult, pSrcItem, pDstItem);
        ...
        if (pDstItem == null)
        {
            ...
            {
                List<ItemPosCount> dest = new();
                var inventoryResult = CanStoreItem(dstbag, dstslot, dest, pSrcItem);

                if (inventoryResult != InventoryResult.Ok)
                {
                    SendEquipError(inventoryResult, pSrcItem);
        ...
        {
            InventoryResult canEquipItem;
            List<ItemPosCount> posCounts = new();
            ushort pos = 0;
            ...
            // can be merge/fill
            if (canEquipItem == InventoryResult.Ok)
            {
    ...
    public void ToggleMetaGemsActive(uint exceptslot, bool apply)
    {
        ...
        for (var slot = EquipmentSlot.Start; slot < EquipmentSlot.End; ++slot)
        {
            ...
            for (var enchantSlot = EnchantmentSlot.Sock1; enchantSlot < EnchantmentSlot.Sock1 + 3; ++enchantSlot)
            {
                ...
                if (condition != 0)
                    ApplyEnchantment(pItem, enchantSlot, apply);
            }
        }
    }
    ...
    public void UpdateAverageItemLevelTotal()
    {
        ...
        ForEachItem(ItemSearchLocation.Everywhere,
                    item =>
                    {
                        ...
                        if (itemTemplate is { InventoryType: < InventoryType.ProfessionTool })
                        {
                            if (item.IsEquipped)
                            {
                                var itemLevel = item.GetItemLevel(this);
                            ...
                            else if (CanEquipItem(ItemConst.NullSlot, out _, item, true, false) == InventoryResult.Ok)
                            {
                                var itemLevel = item.GetItemLevel(this);
                                var inventoryType = itemTemplate.InventoryType;
    ...
    private bool _StoreOrEquipNewItem(uint vendorslot, uint item, byte count, byte bag, byte slot, long price, ItemTemplate pProto, Creature pVendor, VendorItem crItem, bool bStore)
    {
        ...
        if (it != null)
        {
            var newCount = pVendor.UpdateVendorItemCurrentCount(crItem, count);

            BuySucceeded packet = new()
            {
                VendorGUID = pVendor.GUID,
                Muid = vendorslot + 1,
                NewQuantity = crItem.Maxcount > 0 ? newCount : 0xFFFFFFFF,
                QuantityBought = count
            };

            SendPacket(packet);

            SendNewItem(it, count, true, false);
    ...
    {
        var powerSpell = SpellManager.GetSpellInfo(azeriteEssencePower.MinorPowerDescription);

        if (powerSpell != null)
        {
            if (apply)
                SpellFactory.CastSpell(item, powerSpell.Id);
            else
        ...
        powerSpell = SpellManager.GetSpellInfo(azeriteEssencePower.MajorPowerDescription);
    ...
    private void ApplyEquipCooldown(Item pItem)
    {
        ...
        {
            var effectSpellInfo = SpellManager.GetSpellInfo((uint)effectData.SpellID);
            ...
            ItemCooldown data = new()
            {
                ...
                Cooldown = 30 * Time.IN_MILLISECONDS //Always 30secs?
            };
    ...
    private void ApplyItemEquipSpell(Item item, bool apply, bool formChange = false)
    {
        ...
        foreach (var effectData in item.Effects)
        {
            ...
            var spellproto = SpellManager.GetSpellInfo((uint)effectData.SpellID);
    ...
    private void AutoStoreLoot(byte bag, byte slot, uint lootID, LootStore store, ItemContext context = 0, bool broadcast = false, bool createdByPlayer = false)
    {
        var loot = LootFactory.GenerateLoot(null, ObjectGuid.Empty, LootType.None);
        loot.FillLoot(lootID, store, this, true, false, LootModes.Default, context);

        loot.AutoStore(this, bag, slot, broadcast, createdByPlayer);
        UnitCombatHelpers.ProcSkillsAndAuras(this, null, new ProcFlagsInit(ProcFlags.Looted), new ProcFlagsInit(), ProcFlagsSpellType.MaskAll, ProcFlagsSpellPhase.None, ProcFlagsHit.None, null, null, null);
    }
    ...
    private InventoryResult CanStoreItem(byte bag, byte slot, List<ItemPosCount> dest, uint entry, uint count, Item pItem, bool swap, out uint noSpaceCount)
    {
        noSpaceCount = 0;
        Log.Logger.Debug("STORAGE: CanStoreItem bag = {0}, slot = {1}, item = {2}, count = {3}", bag, slot, entry, count);

        var pProto = ItemTemplateCache.GetItemTemplate(entry);
        ...
        // check count of items (skip for auto move for same player from bank)
        uint noSimilarCount = 0; // can't store this amount similar items
        var res = CanTakeMoreSimilarItems(entry, count, pItem, ref noSimilarCount);

        if (res != InventoryResult.Ok)
        {
            if (count == noSimilarCount)
            {
                noSpaceCount = noSimilarCount;

                return res;
            }

            count -= noSimilarCount;
        }
        ...
        if (pItem is { IsNotEmptyBag: true })
            return InventoryResult.BagInBag;
    ...
    private InventoryResult CanStoreItem_InBag(byte bag, List<ItemPosCount> dest, ItemTemplate pProto, ref uint count, bool merge, bool nonSpecialized, Item pSrcItem, byte skipBag, byte skipSlot)
    {
        ...
        for (byte j = 0; j < pBag.GetBagSize(); j++)
        {
            ...
            var needSpace = pProto.MaxStackSize;
            ...
            if (pItem2 != null)
            {
                ...
                needSpace -= pItem2.Count;
            }

            if (needSpace > count)
                needSpace = count;

            ItemPosCount newPosition = new((ushort)(bag << 8 | j), needSpace);
    ...
    private InventoryResult CanStoreItem_InInventorySlots(byte slotBegin, byte slotEnd, List<ItemPosCount> dest, ItemTemplate pProto, ref uint count, bool merge, Item pSrcItem, byte skipBag, byte skipSlot)
    {
        //this is never called for non-bag slots so we can do this
        if (pSrcItem is { IsNotEmptyBag: true })
            return InventoryResult.DestroyNonemptyBag;

        for (var j = slotBegin; j < slotEnd; j++)
        {
            // skip specific slot already processed in first called CanStoreItem_InSpecificSlot
            if (InventorySlots.Bag0 == skipBag && j == skipSlot)
                continue;
            ...
            ItemPosCount newPosition = new((ushort)(InventorySlots.Bag0 << 8 | j), needSpace);
    ...
    private InventoryResult CanStoreItem_InSpecificSlot(byte bag, byte slot, List<ItemPosCount> dest, ItemTemplate pProto, ref uint count, bool swap, Item pSrcItem)
    {
        ...
        else
        {
            ...
            needSpace = pProto.MaxStackSize - pItem2.Count;
        }

        if (needSpace > count)
            needSpace = count;

        ItemPosCount newPosition = new((ushort)(bag << 8 | slot), needSpace);
    ...
    private InventoryResult CanTakeMoreSimilarItems(uint entry, uint count, Item pItem, ref uint noSpaceCount)
    {
        uint notused = 0;

        return CanTakeMoreSimilarItems(entry, count, pItem, ref noSpaceCount, ref notused);
    }
    ...
    private InventoryResult CanTakeMoreSimilarItems(uint entry, uint count, Item pItem, ref uint noSpaceCount, ref uint offendingItemId)
    {
        ...
        if (pItem is { LootGenerated: true })
            return InventoryResult.LootGone;
        ...
        if (pProto.MaxCount > 0)
        {
            ...
            {
                noSpaceCount = count + curcount - pProto.MaxCount;
        ...
        if (pProto.ItemLimitCategory != 0)
        {
            ...
            if (limitEntry.Flags == 0)
            {
                ...
                {
                    noSpaceCount = count + curcount - limitQuantity;
    ...
    private void DestroyZoneLimitedItem(bool update, uint newZone)
    {
        Log.Logger.Debug("STORAGE: DestroyZoneLimitedItem in map {0} and area {1}", Location.MapId, newZone);
        ...
        for (var i = InventorySlots.ItemStart; i < inventoryEnd; i++)
        {
            ...
            if (pItem.IsLimitedToAnotherMapOrZone(Location.MapId, newZone))
                DestroyItem(InventorySlots.Bag0, i, update);
        }
    ...
    private bool HasGemWithLimitCategoryEquipped(uint limitCategory, uint count, byte exceptSlot)
    {
        uint tempcount = 0;
    ...
    private bool HasItemWithLimitCategoryEquipped(uint limitCategory, uint count, byte exceptSlot)
    {
        uint tempcount = 0;
    ...
    private void UpdateItemDuration(uint time, bool realtimeonly = false)
    {
        ...
        foreach (var item in _itemDuration.Where(item => !realtimeonly || item.Template.HasFlag(ItemFlags.RealDuration)))
