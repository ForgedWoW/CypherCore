...
public partial class Player : Unit
{
    ...
    public int CalculateReputationGain(ReputationSource source, uint creatureOrQuestLevel, int rep, int faction, bool noQuestBonus = false)
    {
        ...
        switch (source)
        {
            case ReputationSource.Kill:
                rate = GetDefaultValue("Rate.Reputation.LowLevel.Kill", 1.0f);
        ...
        if (source != ReputationSource.Spell && GetsRecruitAFriendBonus(false))
            percent *= 1.0f + GetDefaultValue("Rate.Reputation.RecruitAFriendBonus", 0.1f);
    ...
    public bool Create(ulong guidlow, CharacterCreateInfo createInfo)
    {
        ...
        if (!Players.PlayerComputators.IsValidGender(createInfo.Sex))
        {
    ...
    public double EnvironmentalDamage(EnviromentalDamage type, double damage)
    {
        ...
        v finalDamage = Unit.DealDamage(this, damage, null, DamageEffectType.Self, dmgSchool, null, false);
        ...
        if (!IsAlive)
        {
            ...
            {
                Log.Logger.Debug($"Player::EnvironmentalDamage: Player '{GetName()}' ({GUID}) fall to death, losing {GetDefaultValue("DurabilityLoss.OnDeath", 10.0f)} durability");
                DurabilityLossAll(GetDefaultValue("DurabilityLoss.OnDeath", 10.0f) / 100, false);
                // durability lost message
                SendDurabilityLoss(this, (uint)GetDefaultValue("DurabilityLoss.OnDeath", 10.0f)));
            }
    ...
    public uint GetCorpseReclaimDelay(bool pvp)
    {
        if (pvp)
        {
            if (!GetDefaultValue("Death.CorpseReclaimDelay.PvP", true))
                return PlayerConst.copseReclaimDelay[0];
        }
        else if (!GetDefaultValue("Death.CorpseReclaimDelay.PvE", true))
        {
            return 0;
        }
    ...
    public uint GetCurrencyMaxQuantity(CurrencyTypesRecord currency, bool onLoad = false, bool onUpdateVersion = false)
    {
        ...
        if (currency.MaxQtyWorldStateID != 0)
            maxQuantity = (uint)Global.WorldStateMgr.GetValue(currency.MaxQtyWorldStateID, Location.Map);
    ...
    public uint GetGossipTextId(WorldObject source)
    {
        ...
        return GetGossipTextId(Players.PlayerComputators.GetDefaultGossipMenuForSource(source), source);
    }
    ...
    public Creature GetNPCIfCanInteractWith(ObjectGuid guid, NPCFlags npcFlags, NPCFlags2 npcFlags2)
    {
        ...
        if (!creature.Location.IsWithinDistInMap(this, creature.CombatReach + 4.0f))
            return null;
    ...
    public void RemovePet(Pet pet, PetSaveMode mode, bool returnreagent = false)
    {
        ...
        if (returnreagent && (pet || TemporaryUnsummonedPetNumber != 0) && !InBattleground)
        {
        ...
        if (pet == null)
        {
            ...
            if (mode == PetSaveMode.NotInSlot && PetStable1 is { CurrentPetIndex: { } })
    ...
    public void RepopAtGraveyard()
    {
        ...
        {
            var bf = Global.BattleFieldMgr.GetBattlefieldToZoneId(Location.Map, Location.Zone);
    ...
    public void SetDrunkValue(byte newDrunkValue, uint itemId = 0)
    {
        ...
        var oldDrunkenState = Players.PlayerComputators.GetDrunkenstateByValue(DrunkValue);
        ...
        var newDrunkenState = Players.PlayerComputators.GetDrunkenstateByValue(newDrunkValue);
    ...
    public void SpawnCorpseBones(bool triggerSave = true)
    {
        ...
        if (Location.Map.ConvertCorpseToBones(GUID))
    ...
    public bool TeleportTo(uint mapid, float x, float y, float z, float orientation, TeleportToOptions options = 0, uint? instanceId = null)
    {
        ...
        if (Location.MapId == mapid && (!instanceId.HasValue || Location.Map?.InstanceId == instanceId))
        {
            ...
            if (!options.HasAnyFlag(TeleportToOptions.NotUnSummonPet))
                ...
                if (pet && !pet.Location.IsWithinDist3d(x, y, z, Location.Map.VisibilityRange))
                    UnsummonPetTemporaryIfAny();
    ...
    {
        if (!pOther || !Location.IsInMap(pOther))
            return false;
    ...
    public override bool UpdatePosition(float x, float y, float z, float orientation, bool teleport = false)
    {
        ...
        if (GetTrader() && !Location.IsWithinDistInMap(GetTrader(), SharedConst.InteractionDistance))
