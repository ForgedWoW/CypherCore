...
public partial class Player
{
    public void _LoadMail(SQLResult mailsResult, SQLResult mailItemsResult, SQLResult artifactResult, SQLResult azeriteItemResult, SQLResult azeriteItemMilestonePowersResult, SQLResult azeriteItemUnlockedEssencesResult, SQLResult azeriteEmpoweredItemResult)
    {
        ...
        if (!mailItemsResult.IsEmpty())
        {
            ...
            do
            {
                ...
                Players.PlayerComputators._LoadMailedItem(GUID, this, mailId, mailById[mailId], mailItemsResult.GetFields(), additionalData.LookupByKey(mailItemsResult.Read<ulong>(0)));
    ...
    public bool LoadFromDB(ObjectGuid guid, SQLQueryHolder<PlayerLoginQueryLoad> holder)
    {
        ...
        else if (map.IsDungeon) // if map is dungeon...
        {
            ...
            else if (instanceID != 0 && Global.InstanceLockMgr.FindActiveInstanceLock(guid, new MapDb2Entries(mapId, map.DifficultyID)) != null) // ... and instance is reseted then look for entrance.
            {
            ...
            do
            {
                ...
                if (item != null)
                {
                    ...
                    if (bagGuid.IsEmpty)
                    {
                        ...
                        if (Players.PlayerComputators.IsInventoryPos(InventorySlots.Bag0, slot))
                        {
                        ...
                        else if (Players.PlayerComputators.IsEquipmentPos(InventorySlots.Bag0, slot))
                        {
                        ...
                        else if (Players.PlayerComputators.IsBankPos(InventorySlots.Bag0, slot))
                        {
                            List<ItemPosCount> dest = new();
                        ...
                        if (err == InventoryResult.Ok)
                        {
                            if (Players.PlayerComputators.IsBagPos(item.Pos))
                            {
                                var pBag = item.AsBag;
                        ...
                        else if (Players.PlayerComputators.IsBagPos(item.Pos))
                        {
                            if (item.IsBag)
    ...
    private void _SaveCustomizations(SQLTransaction trans)
    {
        ...
        Players.PlayerComputators.SavePlayerCustomizations(trans, GUID.Counter, PlayerData.Customizations);
    }
