...
public class LFGManager
{
                      ...
                      InstanceLockManager instanceLockManager, GameEventManager gameEventManager, WorldManager worldManager, ClassFactory classFactory)
    {
        ...
        _ = classFactory.Resolve<LFGGroupScript>();
    }

    public LfgOptions Options
    {
        get => _options;
        set => _options = value;
    }
    ...
    public bool CheckGroupRoles(Dictionary<ObjectGuid, LfgRoles> groles)
    {
        ...
        {
            var role = groles[key] & ~LfgRoles.Leader;
            ...
            if (role.HasAnyFlag(LfgRoles.Damage))
            {
                ...
                {
                    groles[key] -= LfgRoles.Damage;

                    if (CheckGroupRoles(groles))
                        return true;

                    groles[key] += (byte)LfgRoles.Damage;
                }
            ...
            if (role.HasAnyFlag(LfgRoles.Healer))
            {
                ...
                {
                    groles[key] -= LfgRoles.Healer;

                    if (CheckGroupRoles(groles))
                        return true;

                    groles[key] += (byte)LfgRoles.Healer;
                }
            ...
            {
                groles[key] -= LfgRoles.Tank;

                if (CheckGroupRoles(groles))
                    return true;

                groles[key] += (byte)LfgRoles.Tank;
            }
    ...
    public string ConcatenateDungeons(List<uint> dungeons)
    {
        ...
        foreach (var id in dungeons)
            dungeonstr.Append(dungeonstr.Capacity != 0 ? $", {id}" : $"{id}");
    ...
    public void FinishDungeon(ObjectGuid gguid, uint dungeonId, Map currMap)
    {
        ...
        foreach (var guid in players)
        {
            ...
            player.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgPlayerReward(data);
        }
    }
    ...
    {
        return GetLFGDungeon(dungeonId)?.Type ?? LfgType.None;
    }
    ...
    public uint GetLFGDungeonEntry(uint id)
    {
        ...
        return GetLFGDungeon(id)?.Entry() ?? 0;
    }
    ...
    public Dictionary<uint, LfgLockInfoData> GetLockedDungeons(ObjectGuid guid)
    {
        ...
        foreach (var it in dungeons)
        {
            ...
            if (denyJoin)
                ...
                lockStatus = LfgLockStatusType.RaidLocked;
            else if (dungeon.Difficulty > Difficulty.Normal && _instanceLockManager.FindActiveInstanceLock(guid, new MapDb2Entries(dungeon.Map, dungeon.Difficulty, _cliDB, _db2Manager)) != null)
                lockStatus = LfgLockStatusType.RaidLocked;
    ...
    {
        return _queuesStore.TryGetValue(GetQueueId(guid), out var lfgQueue) ? lfgQueue.GetJoinTime(guid) : 0;
    }
    ...
    public uint GetSelectedRandomDungeon(ObjectGuid guid)
    {
        ...
        return dungeon is { Type: LfgType.Raid } ? dungeons.First() : 0;
    }
    ...
    public void InitBoot(ObjectGuid gguid, ObjectGuid kicker, ObjectGuid victim, string reason)
    {
        ...
        var boot = new LfgPlayerBoot
        {
            InProgress = true,
            CancelTime = GameTime.CurrentTime + SharedConst.LFGTimeBoot,
            Reason = reason,
            Victim = victim
        };

        _bootsStore[gguid] = boot;
    ...
    public bool InLfgDungeonMap(ObjectGuid guid, uint map, Difficulty difficulty)
    {
        ...
        var dungeon = GetLFGDungeon(dungeonId);
        return dungeon != null && dungeon.Map == map && dungeon.Difficulty == difficulty;
    }
    ...
    {
        if (player?.Session == null || dungeons.Empty())
            return;
        ...
        uint rDungeonId = 0;
        var isContinue = grp is { IsLFGGroup: true } && GetState(gguid) != LfgState.FinishedDungeon;
        ...
        if (joinData.Result != LfgJoinResult.Ok)
        {
            Log.Logger.Debug("Join: [{0}] joining with {1} members. result: {2}", guid, grp?.MembersCount ?? 1, joinData.Result);

            if (!dungeons.Empty()) // Only should show lockmap when have no dungeons available
                joinData.Lockmap.Clear();

            player.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgJoinResult(joinData);
        ...
        if (grp != null) // Begin rolecheck
        {
            ...
            for (var refe = grp.FirstMember; refe != null; refe = refe.Next())
            {
                ...
                var pguid = plrg.GUID;
                plrg.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgUpdateStatus(updateData, true);
        ...
        else // Add player to queue
        {
            ...
            SetRoles(guid, roles);
            player.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgUpdateStatus(new LfgUpdateData(LfgUpdateType.JoinQueueInitial, dungeons), false);
            SetState(gguid, LfgState.Queued);
            player.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgUpdateStatus(new LfgUpdateData(LfgUpdateType.AddedToQueue, dungeons), false);
            player.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgJoinResult(joinData);
            debugNames += player.GetName();
        }

        StringBuilder o = new();
        o.Append($"Join: [{guid}] joined (group{debugNames}) Members: {dungeons.Count}. Dungeons ({ConcatenateDungeons(dungeons)}): ");
    ...
    public void LoadLFGDungeons(bool reload = false)
    {
        ...
        foreach (var dungeon in _cliDB.LFGDungeonsStorage.Values.Where(dungeon => _db2Manager.GetMapDifficultyData((uint)dungeon.MapID, dungeon.DifficultyID) != null))
    ...
    public void RemoveGroupData(ObjectGuid guid)
    {
        ...
        foreach (var playerGuid in players)
        {
            ...
            SetState(playerGuid, LfgState.None);
            SendLfgUpdateStatus(playerGuid, new LfgUpdateData(LfgUpdateType.RemovedFromQueue), true);
        }
    ...
    public bool SelectedRandomLfgDungeon(ObjectGuid guid)
    {
        ...
        return dungeon != null && (dungeon.Type == LfgType.Random || dungeon.Seasonal);
    }

    public void SendLfgBootProposalUpdate(ObjectGuid guid, LfgPlayerBoot boot)
    {
        _objectAccessor.FindPlayer(guid)?.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgBootProposalUpdate(boot);
    }

    public void SendLfgJoinResult(ObjectGuid guid, LfgJoinResultData data)
    {
        _objectAccessor.FindPlayer(guid)?.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgJoinResult(data);
    }

    public void SendLfgQueueStatus(ObjectGuid guid, LfgQueueStatusData data)
    {
        _objectAccessor.FindPlayer(guid)?.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgQueueStatus(data);
    }

    public void SendLfgRoleCheckUpdate(ObjectGuid guid, LfgRoleCheck roleCheck)
    {
        _objectAccessor.FindPlayer(guid)?.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgRoleCheckUpdate(roleCheck);
    }

    public void SendLfgRoleChosen(ObjectGuid guid, ObjectGuid pguid, LfgRoles roles)
    {
        _objectAccessor.FindPlayer(guid)?.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgRoleChosen(pguid, roles);
    }

    public void SendLfgUpdateProposal(ObjectGuid guid, LfgProposal proposal)
    {
        _objectAccessor.FindPlayer(guid)?.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgProposalUpdate(proposal);
    }

    public void SendLfgUpdateStatus(ObjectGuid guid, LfgUpdateData data, bool party)
    {
        _objectAccessor.FindPlayer(guid)?.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgUpdateStatus(data, party);
    }
    ...
    public void TeleportPlayer(Player player, bool outt, bool fromOpcode = false)
    {
        ...
        if (dungeon == null)
        {
            ...
            player.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgTeleportError(LfgTeleportResult.NoReturnLocation);
        ...
        else if (player.Location.MapId != dungeon.Map) // Do not teleport players in dungeon to the entrance
        {
            ...
            if (!fromOpcode)
                ...
                for (var refe = group.FirstMember; refe != null; refe = refe.Next())
                {
                    ...
                    if (plrg == null || plrg == player || plrg.Location.MapId != dungeon.Map)
                        continue;
        ...
        if (error != LfgTeleportResult.None)
            player.Session.PacketRouter.OpCodeHandler<LFGHandler>().SendLfgTeleportError(error);
    ...
    public void Update(uint diff)
    {
        ...
        foreach (var itBoot in _bootsStore)
        {
            ...
            foreach (var pguid in boot.Votes.Select(itVotes => itVotes.Key).Where(pguid => pguid != boot.Victim))
    ...
    public void UpdateProposal(uint proposalId, ObjectGuid guid, bool accept)
    {
        ...
        LfgProposalPlayer player = null;

        if (proposal?.Players.TryGetValue(guid, out player) == false)
            return;
        ...
        foreach (var it in proposal.Players)
        {
            ...
            var connectedPlayer = _objectAccessor.FindConnectedPlayer(pguid);

            var group = connectedPlayer?.Group;
    ...
    public void UpdateRoleCheck(ObjectGuid gguid, ObjectGuid guid = default, LfgRoles roles = LfgRoles.None)
    {
        ...
        else
        {
            ...
            if (done)
            {
                ...
                var checkRoles = roleCheck.Roles;
        ...
        foreach (var guid in players)
        {
            ...
            foreach (var it2 in cachedLockMap)
            {
                ...
                if (dungeons.Contains(dungeonId))
                {
                    ...
                    {
                        var dungeon = GetLFGDungeon(dungeonId);
                        MapDb2Entries entries = new(dungeon.Map, dungeon.Difficulty, _cliDB, _db2Manager);
    ...
    private void MakeNewGroup(LfgProposal proposal)
    {
        ...
        foreach (var pguid in players)
        {
            ...
            if (grp == null)
            {
                grp = _classFactory.Resolve<PlayerGroup>();
                grp.ConvertToLFG();
                grp.Create(player);
                var gguid = grp.GUID;
                SetState(gguid, LfgState.Proposal);
            ...
            if (rDungeon is { Type: LfgType.Random })
                player.SpellFactory.CastSpell(player, SharedConst.LFGSpellDungeonCooldown);
        }
        ...
        SetDungeon(grp.GUID, dungeon.Entry());
        SetState(grp.GUID, LfgState.Dungeon);

        _SaveToDB(grp.GUID, grp.DbStoreId);

        // Teleport Player
        foreach (var player in playersToTeleport.Select(it => _objectAccessor.FindPlayer(it)).Where(player => player != null))
    ...
    private void RemoveProposal(KeyValuePair<uint, LfgProposal> itProposal, LfgUpdateType type)
    {
        ...
        if (type == LfgUpdateType.ProposalFailed)
            foreach (var it in proposal.Players.Where(it => it.Value.Accept == LfgAnswer.Pending))
    ...
    private void RestoreState(ObjectGuid guid)
    {
        if (guid.IsParty)
        {
            var data = _groupsStore[guid];
