...
class NPCCommands
{
	...
	static bool HandleNpcInfoCommand(CommandHandler handler)
	{
		...
		var curRespawnDelay = target.RespawnCompatibilityMode ? target.RespawnTimeEx - _gameTime.CurrentGameTime : target.Map.GetCreatureRespawnTime(target.SpawnId) - _gameTime.CurrentGameTime;
		...
		var data = _gameObjectManager.GetCreatureData(target.SpawnId);
	...
	static bool HandleNpcMoveCommand(CommandHandler handler, ulong? spawnId)
	{
		...
		var data = _gameObjectManager.GetCreatureData(lowguid);
		...
		_gameObjectManager.RemoveCreatureFromGrid(data);
		data.SpawnPoint.Relocate(player.Location);
		_gameObjectManager.AddCreatureToGrid(data);

		// update position in DB
		var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.UPD_CREATURE_POSITION);
	...
	static bool HandleNpcNearCommand(CommandHandler handler, float? dist)
	{
		...
		var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.SEL_CREATURE_NEAREST);
		stmt.AddValue(0, player.Location.X);
		stmt.AddValue(1, player.Location.Y);
		stmt.AddValue(2, player.Location.Z);
	...
	static bool HandleNpcWhisperCommand(CommandHandler handler, string recv, Tail text)
	{
		...
		var receiver = _objectAccessor.FindPlayerByName(recv);
	...
	static void _IterateNotNormalLootMap(CommandHandler handler, MultiMap<ObjectGuid, NotNormalLootItem> map, List<LootItem> items)
	{
		foreach (var key in map.Keys)
		{
			...
			var player = _objectAccessor.FindConnectedPlayer(key);
	...
	class AddCommands
	{
		...
		{
			if (_gameObjectManager.GetCreatureTemplate(id) == null)
				return false;
			...
			if (trans)
			{
				var guid = _gameObjectManager.GenerateCreatureSpawnId();
				var data = _gameObjectManager.NewOrExistCreatureData(guid);
				data.SpawnId = guid;
			...
			_gameObjectManager.AddCreatureToGrid(_gameObjectManager.GetCreatureData(db_guid));
		...
		static bool HandleNpcAddVendorItemCommand(CommandHandler handler, uint itemId, uint? mc, uint? it, uint? ec, [OptionalArg] string bonusListIds)
		{
			...
			if (!_gameObjectManager.IsVendorItemValid(vendor_entry, vItem, handler.Session.Player))
				return false;

			_gameObjectManager.AddVendorItem(vendor_entry, vItem);
		...
		static bool HandleNpcAddMoveCommand(CommandHandler handler, ulong lowGuid)
		{
			...
			var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.UPD_CREATURE_MOVEMENT_TYPE);
		...
		static bool HandleNpcAddFormationCommand(CommandHandler handler, ulong leaderGUID)
		{
			...
			var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.INS_CREATURE_FORMATION);
	...
	class DeleteCommands
	{
		...
		static bool HandleNpcDeleteVendorItemCommand(CommandHandler handler, uint itemId)
		{
			...
			if (!_gameObjectManager.RemoveVendorItem(vendor.Entry, itemId, ItemVendorType.Item))
			{
	...
	class SetCommands
	{
		...
		static bool HandleNpcSetFactionIdCommand(CommandHandler handler, uint factionId)
		{
			...
			// ..and DB
			var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.UPD_CREATURE_FACTION);
		...
		static bool HandleNpcSetFlagCommand(CommandHandler handler, NPCFlags npcFlags, NPCFlags2 npcFlags2)
		{
			...
			var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.UPD_CREATURE_NPCFLAG);
		...
		static bool HandleNpcSetLinkCommand(CommandHandler handler, ulong linkguid)
		{
			...
			if (!_gameObjectManager.SetCreatureLinkedRespawn(creature.SpawnId, linkguid))
			{
		...
		static bool HandleNpcSetWanderDistanceCommand(CommandHandler handler, float option)
		{
			...
			var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.UPD_CREATURE_WANDER_DISTANCE);
		...
		static bool HandleNpcSetSpawnTimeCommand(CommandHandler handler, uint spawnTime)
		{
			...
			var stmt = _worldDatabase.GetPreparedStatement(WorldStatements.UPD_CREATURE_SPAWN_TIME_SECS);
