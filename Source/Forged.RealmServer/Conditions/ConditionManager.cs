...
public sealed class ConditionManager
{
    ...
    public ConditionManager(GameObjectManager objectManager,
                                ...
                                AreaTriggerDataStorage areaTriggerDataStorage,
                                ConversationDataStorage conversationDataStorage,
	...
	public void LoadConditions(bool isReload = false)
	{
		...
		do
		{
			Condition cond = new(_cliDb, _achievementGlobalMgr, this, _gameObjectManager, _scriptManager);
	...
	public bool IsPlayerMeetingCondition(Player player, PlayerConditionRecord condition)
	{
		...
		{
			var worldStateExpression = _cliDb.WorldStateExpressionStorage.LookupByKey((uint)condition.WorldStateExpressionID);
		...
		if (condition.MinExpansionLevel != -1 && 
            ...
            ((condition.MinExpansionLevel == _configuration.GetDefaultValue("Expansion", (int)Expansion.Dragonflight) 
              && condition.MinExpansionTier > 0) /*TODO: implement tier*/ || 
             condition.MinExpansionLevel > _configuration.GetDefaultValue("Expansion", (int)Expansion.Dragonflight)))
			return false;
