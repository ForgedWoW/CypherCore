...
public class BattlepayManager
{
	...
	public uint GetBattlePayCredits()
	{
		...
		var result_don = _loginDatabase.Query(stmt);
	...
	public void SendBattlePayBattlePetDelivered(ObjectGuid petguid, uint creatureID)
	{
		...
		Log.Logger.Information("Send BattlePayBattlePetDelivered guid: {} && creatureID: {}", petguid.Counter, creatureID);
	}
	
	public uint GetShopCurrency()
	{
		return (uint)_configuration.GetDefaultValue("FeatureSystem.BpayStore.Currency", 1);
	}
	...
	public void SavePurchase(Purchase purchase)
	{
		...
		stmt.AddValue(1, _worldManager.VirtualRealmAddress);
    ...
    public void SendBattlePayDistribution(uint productId, ushort status, ulong distributionId, ObjectGuid targetGuid)
	{
		...
		if (!targetGuid.IsEmpty)
		{
			...
			distributionBattlePay.DistributionObject.TargetVirtualRealm = _worldManager.VirtualRealmAddress;
			distributionBattlePay.DistributionObject.TargetNativeRealm = _worldManager.VirtualRealmAddress;
		}
