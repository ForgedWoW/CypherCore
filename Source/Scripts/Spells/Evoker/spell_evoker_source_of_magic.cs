...
public class spell_evoker_source_of_magic : SpellScript, ISpellCheckCast, ISpellOnCast
{
    ...
    {
        if (ExplTargetUnit == null
            || !ExplTargetUnit.TryGetAsPlayer(out var player)
            || !Caster.IsInPartyWith(player) 
            || player.GetPrimarySpecialization() == 0
            || CliDB.ChrSpecializationStorage.LookupByKey(player.GetPrimarySpecialization()).Role != 1)
